...
The station-address check (\S5.6) compares the incident-derived footprints with postcode centroids from the historical June 2017 file. It supplies a second reference point, not a current station register.
...
F4 & Partial & Footprint API and report-side historical address/proximity evidence & Station context in the dashboard UI \\
...
\begin{itemize}
    ...
    \item The empty-cell question is hypothetical because all 2{,}574 cells are populated, and the station prompt evaluates the report evidence package rather than the dashboard screen.
\end{itemize}
...
The useful evidence is how participants compared panels and qualified claims. Unsupported task mechanics are excluded from the system-completion claim.
