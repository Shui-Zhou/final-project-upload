...
The two build scripts regenerate the untracked parquet artefacts from the raw LFB workbook under \path{data/}. The raw file is excluded because of repository size and source-refresh cadence, not because OGL v2 forbids redistribution; its URL and SHA-256 are recorded in the provenance sidecar. The dashboard is then served at \path{http://127.0.0.1:5057/dashboard}.
...
Station evidence is report-side and API-side; the dashboard has no station layer. Assignment footprints locate historical responsibility centroids, not buildings. The postcode coordinates come from a historical source file, not a current estate register.
