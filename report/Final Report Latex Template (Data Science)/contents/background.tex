...
The two coordinate families must not be conflated. Precise per-incident coordinates (\texttt{Easting\_m}, \texttt{Northing\_m}) are valid for about 37.9\% of rows; disclosure control accounts for much of the remainder, and the missingness may be systematic. Rounded coordinates (\texttt{Easting\_rounded}, \texttt{Northing\_rounded}) are nearly complete, but rounding introduces displacement and does not guarantee unbiased fine-scale spatial estimates. Precise points therefore support subset-only density diagnostics, while rounded points can support coarser grid checks with an explicit proxy-error caveat. Neither coordinate family limits borough counts or response summaries, which use the published borough field directly. Table~\ref{tab:evidence-support} records these claim boundaries.
