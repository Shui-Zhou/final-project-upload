...
3. Run `python -m pytest tests/ -q`; expect 79 passed.
...
| C3 | The six-minute reference is visually distinct from the data on the distribution pane. | A dashed line at six minutes is labelled as a reference, not an incident-level service target. | §6.2 row 3 | Major |
...
| A2 | Statistical uncertainty is shown beside borough proportions. | The ranking table includes recorded n and a 95% Wilson interval; n < 30 and the partial final month are marked. Coordinate availability is kept out because borough aggregates use the borough field. | §6.5 | Major |
...
| F2 | Temporal pattern → trend pane: line chart, not bar chart, across the observed months. | The trend pane uses a connected line and marks February 2026 as partial. | §6.2 row 2 | Minor |
